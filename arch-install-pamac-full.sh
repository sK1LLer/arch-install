...
pacman-key --init && pacman-key --populate && cd /home/$user_name && sudo -u $user_name git clone https://aur.archlinux.org/yay.git && cd yay && sudo -u $user_name makepkg -si --noconfirm && sudo -u $user_name yay -Sy pamac-full pamac-tray-icon-plasma --noconfirm
...
#umount /.snapshots;rm -r /.snapshots;sudo -u $user_name yay -Sy snapper-support btrfs-assistant --noconfirm;snapper -c root create-config /;sed -i 's|TIMELINE_LIMIT_HOURLY=.*|TIMELINE_LIMIT_HOURLY=\"0\"|' /etc/snapper/configs/root;sed -i 's|TIMELINE_LIMIT_DAILY=.*|TIMELINE_LIMIT_DAILY=\"3\"|' /etc/snapper/configs/root;sed -i 's|TIMELINE_LIMIT_WEEKLY=.*|TIMELINE_LIMIT_WEEKLY=\"0\"|' /etc/snapper/configs/root;sed -i 's|TIMELINE_LIMIT_MONTHLY=.*|TIMELINE_LIMIT_MONTHLY=\"0\"|' /etc/snapper/configs/root;sed -i 's|TIMELINE_LIMIT_QUARTERLY=.*|TIMELINE_LIMIT_QUARTERLY=\"0\"|' /etc/snapper/configs/root;sed -i 's|TIMELINE_LIMIT_YEARLY=.*|TIMELINE_LIMIT_YEARLY=\"0\"|' /etc/snapper/configs/root;systemctl enable snapperd.service
#echo -e "sudo pacman-key --init && sudo pacman-key --populate archlinux && sudo pacman -Syu && cd ~/ && git clone https://aur.archlinux.org/yay.git && cd yay && makepkg -si --noconfirm\nyay -Sy pamac-full" --noconfirm > /home/$user_name/install_yay-pamac.sh
echo -e "yay -Sy snapper-support btrfs-assistant --noconfirm\nsudo umount /.snapshots\nsudo rm -r /.snapshots\nsudo snapper -c root create-config /\nsudo btrfs subvolume delete -i \\\$(sudo btrfs subvolume list / | grep \\"\\.snapshots\\" | awk -F ' ' '{print \\\$2}') /\nsudo sed -i 's|TIMELINE_LIMIT_HOURLY=.*|TIMELINE_LIMIT_HOURLY=\"0\"|' /etc/snapper/configs/root\nsudo sed -i 's|TIMELINE_LIMIT_DAILY=.*|TIMELINE_LIMIT_DAILY=\"3\"|' /etc/snapper/configs/root\nsudo sed -i 's|TIMELINE_LIMIT_WEEKLY=.*|TIMELINE_LIMIT_WEEKLY=\"0\"|' /etc/snapper/configs/root\nsudo sed -i 's|TIMELINE_LIMIT_MONTHLY=.*|TIMELINE_LIMIT_MONTHLY=\"0\"|' /etc/snapper/configs/root\nsudo sed -i 's|TIMELINE_LIMIT_QUARTERLY=.*|TIMELINE_LIMIT_QUARTERLY=\"0\"|' /etc/snapper/configs/root\nsudo sed -i 's|TIMELINE_LIMIT_YEARLY=.*|TIMELINE_LIMIT_YEARLY=\"0\"|' /etc/snapper/configs/root\nsudo mount -a\nsudo systemctl restart snapperd.service\necho 'root = \"@snapshots,@,'$(blkid -s UUID -o value /dev/$root_disk_name)'\"' >> /etc/btrfs-assistant.conf" > /home/$user_name/backups.sh
